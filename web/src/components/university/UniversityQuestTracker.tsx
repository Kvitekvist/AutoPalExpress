...
export function UniversityQuestTracker() {
  ...
  const refresh = React.useCallback(() => universityApi.getCatalog().then(setCatalog).catch(() => {}), []);
  React.useEffect(() => {
    refresh();
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between text-gold-300">
        <span className="flex items-center gap-2 font-display text-sm font-bold"><GraduationCap className="h-4 w-4" /> APE University</span>
        <span className="text-xs text-parchment-300/60">{done}/{course.steps.length}</span>
      </div>
      ...
      <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-stone-700">
        <div className="h-full bg-gradient-to-r from-gold-600 to-gold-300" style={{ width: `${(done / course.steps.length) * 100}%` }} />
      </div>
    </button>
  );
}
