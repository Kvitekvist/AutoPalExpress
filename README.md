...
- **Remote admin access is plain HTTP, not HTTPS.** Fine for a home network with people you trust.
- Windows only.
...
**A friend's invite code doesn't work.** Codes are single-use. heck Settings → Users & Access to see if it's already been claimed, and generate a fresh one if needed.
