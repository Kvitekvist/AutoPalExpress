...
* Process adoption on backend startup is still missing: if Palworld keeps running while the admin backend restarts, the local process manager forgets it until relaunched through the tool, even though the REST API may be reachable.
