...
Fresh-server deployment is managed by `deploy_jobs.py` as a background job with explicit initialize, Steam connection, download/install, configuration, and registration phases. These backend-owned states let the wizard show truthful current progress and the exact failure location without parsing or guessing from SteamCMD log text.
