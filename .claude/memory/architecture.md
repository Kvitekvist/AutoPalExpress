...
* **Application update checks are informational and fail closed.** `app/version.py` is the runtime version source used by Nexus headers and the sidebar API; packaging runs `scripts/check_app_version.py` to reject drift from `installer.iss`. `app_update.py` checks only the fixed public `Kvitekvist/AutoPalExpress` latest-release endpoint, caches success for six hours/failure for fifteen minutes, and exposes a read-only authenticated status. The UI only links to the canonical GitHub release page; it never downloads or executes installers.
* **Packaged frontend MIME types are registered explicitly.** Before mounting Starlette static files, `app/main.py` maps `.js` and `.mjs` to `application/javascript`. This avoids Windows registry MIME mappings leaking into Python's `mimetypes` database and causing browsers to block ES modules in the packaged build (GitHub #154); Vite development serving is unaffected.
* **Live save data is never deleted before its replacement is proven good.** `app/services/safe_replace.py` (shared by Save Import and backup restore) copies a replacement directory to a temporary sibling, verifies it, and only then swaps it into place with two fast renames - the live directory is either untouched or fully replaced, never observed mid-copy. Backups additionally record a per-file SHA256 manifest in `meta.json` at creation time so a later Verify/restore can actually detect corruption instead of assuming a backup folder is intact just because it exists.

---

## Testing & CI

Backend: `tests/` (pytest, `pytest.ini` at repo root, deps in `requirements-dev.txt`). Covers auth, role permissions, instance switching, the shared safe-replace primitive (copy-verify-swap, self-heal on swap failure, the critical-error escape hatch), backup create/verify/restore/retention, save import (deep discovery, validation, destination preview, rollback on failure), mod archive extraction (zip-slip/oversized-archive rejection, pak-vs-ue4ss detection), process-discovery matching logic, and packaged frontend serving (SPA fallback, path-traversal rejection), plus route-level wiring tests in `test_automation_routes.py`. Deliberately not covered by automated tests - needs a real Windows host with a real running `PalServer.exe`/interactive desktop, so it stays manually verified like everything else in this project that does: actual process start/stop launch flags, real UAC/firewall/UPnP calls, native folder dialogs, the installer's own GUI wizard.
