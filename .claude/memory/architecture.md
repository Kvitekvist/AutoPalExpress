...
React 19 + Vite SPA (`web/src/`). Pages: Dashboard (now includes the player roster, merged in), Mods (mod install/enable/disable/reorder from Nexus, plus UE4SS management - install-from-file lives on Super Admin instead, not here), Server Control, World Settings (generic `.ini` editor, deliberately excludes the game port - see Super Admin), Settings (role-gated, super admin only: instance provisioning/removal, automation/backups, user/invite management), Super Admin (role-gated: active server's disk path, the one place to view/edit/save the game port, port-forward management for both the admin and game ports, firewall, Nexus account connection, and mod install-from-file uploads). Both Settings and Super Admin are hidden from nav and route-guarded (redirect to Dashboard) for non-super-admins - a regular admin's day-to-day surface is Dashboard, Mods, Server Control, World Settings, and Logs. High-risk/disk-writing actions are deliberately consolidated onto Super Admin rather than conditionally hidden within pages regular admins also use. Styled with Tailwind CSS v4 (CSS-based theme, no `tailwind.config.js`) and custom "fantasy" components (`RuneButton`, `ScrollPanel`, `CrystalStatus`, etc.) wrapping restyled Radix UI primitives.

### Backend

FastAPI app (`app/main.py`). Routes live in `app/routes/*.py`, one file per feature area (auth, users, instances, mods, nexus, network, server_control, server_settings, automation, players, ue4ss). Business logic lives in `app/services/*.py` - routes are thin, services do the real work (process management, RCON, file I/O, external API calls). A background `asyncio` task (`app/services/scheduler.py`, started on FastAPI startup) drives scheduled backups/restarts/messaging.
...
* Palworld RCON client (`app/services/rcon.py`) - Source RCON protocol implemented directly over `asyncio` streams, talks to the live game server for broadcast messages, forced saves, player list, kick/ban.
* Plain HTTP only (no TLS) - see Decision Log.

### Services

Notable ones beyond the obvious CRUD: `process_manager.py` (real process lifecycle for `PalServer.exe`, including killing the whole process tree since the exe is a thin launcher for a child process; intentionally leaves the Palworld server window visible, but still cannot capture that window text because Palworld does not write it to stdout), `palworld_settings.py` (generic `PalWorldSettings.ini` reader/writer - infers field types from formatting rather than hardcoding Palworld's ~108-field schema, so it doesn't go stale across game updates), `scheduler.py` (the automation loop), `backup_service.py`, `steamcmd.py` (deploying brand-new servers - always into `data_dir()/servers/<name>`, computed by `deploy_jobs.default_install_dir()`), `nexus_client.py` / `mod_installer.py` (mod browsing/installing, including MD5-hash verification against Nexus's own catalog for manually-uploaded files), `first_run_setup.py` (applies the installer's one-time seed file - super admin account, Nexus key, an optional first server - on first launch, using the same code paths the UI itself calls), `activity_log.py` and `app_log_reader.py` (Logs page data: app activity JSONL plus AutoPalExpress' own `backend.log` tail).
...
data/ (dev) or %LOCALAPPDATA%\PalworldServerAdmin\data (packaged)
  ...
  nexus.json                         Nexus Mods account connection
```
...
See `memory/tech_stack.md` for the full list and versions. The recurring theme: **avoid new dependencies for things that can be implemented directly** (UPnP, RCON, firewall elevation, password hashing are all stdlib/from-scratch) - this keeps the PyInstaller build simple and avoids dependency-version churn for a small, single-maintainer project.
...
* **Minimal dependencies.** Prefer a from-scratch implementation (RCON, UPnP, firewall elevation) over pulling in a library, when the protocol/mechanism is small enough to implement directly and doing so avoids a new dependency in the PyInstaller build.
* **Fail open to "tell the user clearly," not silently or with a crash.** RCON-unavailable, UPnP-unavailable, and similar "the environment isn't set up for this yet" conditions are modeled as expected, frequent, non-exceptional outcomes with clear user-facing messages - not 500s.
...
* `process_manager`'s "is this server running" tracking is in-memory only and resets on a backend restart, even though the real game process keeps running - causes the tool to show "offline" for a genuinely-running server until it's restarted through the tool. No process adoption exists yet.
* The manual Stop Server / Restart Server actions still use `CTRL_BREAK_EVENT` (never confirmed to trigger a graceful, saved shutdown) rather than the RCON client that now exists - RCON's own `Save` is used as a best-effort pre-restart safety net, but the stop mechanism itself wasn't changed (deliberately scoped out of the automation feature as separate, already-large-enough work).
