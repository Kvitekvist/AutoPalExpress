...
* OPEN (TICKET-0006): Kicking a player from the Dashboard roster completes without error (RCON itself works - Broadcast succeeds) but doesn't actually disconnect them. A real bug was found and fixed on the way (`show_players()` wasn't stripping whitespace from the `ShowPlayers` CSV fields, so `steamid` could be silently wrong) but didn't resolve it. Leading unconfirmed hypothesis: Palworld's `KickPlayer` may expect `playeruid` rather than `steamid` in this game version. Added response logging (`rcon: kick <id> -> <raw response>`) to nail down the actual cause next time it's reproduced - waiting on that log output before changing which ID field gets passed.
