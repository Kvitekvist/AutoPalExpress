...
* OPEN (TICKET-0006): Kicking a player from the Dashboard roster completes without error (RCON itself works - Broadcast succeeds) but doesn't actually disconnect them. Two real bugs found and fixed so far, neither yet confirmed to be the full fix: (1) `show_players()` wasn't stripping whitespace from the `ShowPlayers` CSV fields; (2) the user's own server console showed connecting Steam players as `User id: steam_<SteamID64>`, but `ShowPlayers` returns the bare numeric ID with no prefix - `KickPlayer`/`BanPlayer`/`UnBanPlayer` now prefix a bare numeric ID with `steam_` to match. Waiting on the user to confirm the kick actually works now.
